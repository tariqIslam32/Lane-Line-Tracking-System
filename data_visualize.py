...
class Vis():
  ...
  def set_current_file(self, filename):
        self.index = self.lidar_paths.index(os.path.join(self.data_folder, filename))
        self.points = self.load_points()
        self.lanes = self.load_lanes()

  def close_window(self):
        gui.Application.instance.quit()  # Modify this based on how Open3D handles window closing
